...
import { createOrganicFormGeometry, createOrbitPath } from './orbit-system.js'
...
const moonMaterial = new THREE.ShaderMaterial({
  ...
  vertexShader: `
    ...
    void main() {
      vec3 p = position;
      float broad = sin(p.x * 2.1 + p.y * 1.7 + time * .08) * cos(p.z * 2.4 - time * .06);
      float breath = sin(time * (.22 + energy * .08)) * .006 + burst * .014;
      p += normal * (broad * turbulence * .004 + breath);
  ...
  fragmentShader: `
    ...
    uniform float time, energy, burst;
    uniform vec3 primary, accent;
    varying vec3 vNormal, vWorld, vView;
    float hash(vec3 p) {
      p = fract(p * .3183099 + .1);
      p *= 17.;
    ...
    float fbm(vec3 p) {
      float n = 0., a = .55;
      for (int i = 0; i < 4; i++) { n += noise(p) * a; p = p * 2.03 + 1.7; a *= .48; }
      return n;
    }
    void main() {
      ...
      float macro = fbm(normalize(vWorld) * 2.8 + vec3(7.1));
      float detail = noise(vWorld * 8.5 + vec3(3.7)) * .68 + noise(vWorld * 17.1 + vec3(1.9)) * .32;
      ...
      value *= .76 + macro * .32 + detail * .13 - craters * .13;
      value += paper + burst * .035;
      vec3 tint = mix(primary, accent, macro);
      vec3 color = vec3(value) + tint * (.008 + energy * .005);
      color *= mix(.5, 1., limb);
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        float wave = sin(uv.x * 12. + time * (.42 + energy * .15) + seed) * cos(uv.y * 8. - time * .28);
        p.z += wave * (.018 + turbulence * .018) * (.4 + edge) + burst * .045 * sin(uv.y * 9.);
...
const formDefs=[
{
  seed:13,width:1.5,height:1.04,depth:.28,home:[-2.8,2.25,.25],drift:[.18,.1],scale:.92,rotation:[-.34,.26,-.72],rate:.62
},
{
  seed:29,width:1.16,height:1.46,depth:.25,home:[2.15,2.68,.2],drift:[.12,.09],scale:.82,rotation:[.22,-.2,.38],rate:.78
},
{
  seed:47,width:1.08,height:.82,depth:.32,home:[3.05,.72,.38],drift:[.15,.08],scale:.82,rotation:[-.14,.52,-.86],rate:.55
},
{
  seed:61,width:1.58,height:1.28,depth:.34,home:[2.15,-1.9,.85],drift:[.2,.12],scale:.98,rotation:[.4,-.18,.22],rate:.46
},
{
  seed:83,width:1.34,height:1.02,depth:.29,home:[-2.95,-1.76,.62],drift:[.16,.11],scale:.88,rotation:[-.48,.4,.7],rate:.7
}
]
...
const target={
  primary:new THREE.Color(EMOTIONS.wonder.primary),accent:new THREE.Color(EMOTIONS.wonder.accent),energy:EMOTIONS.wonder.energy,turbulence:EMOTIONS.wonder.turbulence,orbitSpeed:EMOTIONS.wonder.orbitSpeed,shape:EMOTIONS.wonder.shape
}
...
function applyVisualState(visual,name,source='control'){
  ...
  Object.assign(target,{
    energy:visual.energy,turbulence:visual.turbulence,orbitSpeed:visual.orbitSpeed,shape:visual.shape
  });
...
function resize(){
  ...
  const renderScale=width>=1200?.85:width>=900?.92:1;
...
function animate(now){
  ...
  moon.rotation.y=elapsed*.007*motion+dragOrbit.rotation.yaw*.12;
  moon.rotation.z=Math.sin(elapsed*.08)*.012*motion;
  const pulse=1+Math.sin(elapsed*(.22+state.energy.value*.06))*.004*motion+burst*.012;
  moon.scale.set(pulse,1.015*pulse,pulse);
  for(let i=0;
  i<orbitLines.length;
  i++)orbitLines[i].line.rotation.y=Math.sin(elapsed*orbitLines[i].speed)*.018*motion;
  for(let i=0;
  i<orbiters.length;
  i++){
    const o=orbiters[i],driftTime=elapsed*orbitSpeed*o.rate*.08*motion+o.phase;
    o.mesh.position.x=o.home[0]+Math.cos(driftTime)*o.drift[0];
    o.mesh.position.y=o.home[1]+Math.sin(driftTime)*o.drift[1];
    o.mesh.position.z=o.home[2]+Math.sin(driftTime*.7)*.06;
    o.mesh.rotation.z+=delta*(.025+i*.008)*motion;
    o.mesh.rotation.y+=delta*(i%2?-.018:.021)*motion;
    const s=1+visualShape*(i%2?.025:-.018);
    o.mesh.scale.setScalar(o.scale*s*(1+burst*.025))
  }
